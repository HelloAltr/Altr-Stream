"""Cross-database semantic parity tests for AltrQL across SQLite, PostgreSQL, and MySQL.

Verifies that identical AltrQL queries produce equivalent logical results across all 3 physical targets.
"""
...
async def test_cross_database_semantic_parity(client: AsyncClient):
    """Execute identical AltrQL queries across SQLite, PostgreSQL, and MySQL, verifying parity."""
    # 1. Check availability of live Postgres and MySQL containers
    ...
    if not pg_available or not mysql_available:
        pytest.skip("PostgreSQL or MySQL container not available on default test ports")
    ...
    # 5. Register sources in Altr Stream
    ...
    # Helper to execute query on all 3 sources and extract comparable results
    async def run_query(query: str):
        ...
        def normalize_rows(rows):
            # Normalize row list to list of tuples of ID and email for stable comparison
            return [(r.get("id"), r.get("email")) for r in rows]

        return normalize_rows(r_sq.json()["rows"]), normalize_rows(r_pg.json()["rows"]), normalize_rows(r_my.json()["rows"])

    # -----------------------------------------------------------------------
    # Case 1: Simple GET + Default Deterministic Ordering (id ASC)
    # -----------------------------------------------------------------------
    sq_rows, pg_rows, my_rows = await run_query("GET parity_users;")
    ...
    # -----------------------------------------------------------------------
    sq_rows, pg_rows, my_rows = await run_query('GET parity_users WHERE { email = "alice@example.com" };')
    ...
    # -----------------------------------------------------------------------
    sq_rows, pg_rows, my_rows = await run_query('GET parity_users WHERE { status = {"ACTIVE", "PENDING"} };')
    ...
    # -----------------------------------------------------------------------
    sq_rows, pg_rows, my_rows = await run_query("GET parity_users WHERE { age = {19, 25..30} };")
    ...
    # -----------------------------------------------------------------------
    sq_rows, pg_rows, my_rows = await run_query('GET parity_users WHERE { email HAS "example" };')
    ...
    # -----------------------------------------------------------------------
    sq_rows, pg_rows, my_rows = await run_query('GET parity_users WHERE { email HAS {"alice", "corp"} };')
    ...
    # -----------------------------------------------------------------------
    sq_rows, pg_rows, my_rows = await run_query('GET parity_users WHERE { email NOT HAS "example" };')
    ...
    # -----------------------------------------------------------------------
    sq_rows, pg_rows, my_rows = await run_query("GET parity_users WHERE { age = {22..30} };")
    assert sq_rows == [(1, "alice@example.com"), (2, "bob@example.com"), (3, "carol@example.com")]
    assert pg_rows == sq_rows
    assert my_rows == sq_rows
    ...
    # -----------------------------------------------------------------------
    sq_rows, pg_rows, my_rows = await run_query("""
    ...
    # -----------------------------------------------------------------------
    sq_rows, pg_rows, my_rows = await run_query("GET parity_users SORT { age DESC, id ASC };")
    ...
    # -----------------------------------------------------------------------
    sq_rows, pg_rows, my_rows = await run_query("GET parity_users TOP 2 BY id OFFSET 1;")
    ...
    # -----------------------------------------------------------------------
    sq_rows, pg_rows, my_rows = await run_query('GET parity_users WHERE { metadata.department = "Engineering" };')
    assert sq_rows == [(1, "alice@example.com")]
    assert pg_rows == sq_rows
    assert my_rows == sq_rows
    ...
    # -----------------------------------------------------------------------
    sq_rows, pg_rows, my_rows = await run_query("GET parity_users WHERE { metadata = NULL };")
    assert sq_rows == [(4, "dave@corp.net"), (5, "eve@test.org")]
    assert pg_rows == sq_rows
    assert my_rows == sq_rows
