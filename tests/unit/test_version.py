...
def test_canonical_version_constant():
    """Verify the canonical version string matches 0.13.5-alpha across definitions."""
    assert __version__ == "0.13.5-alpha"
    assert VERSION == "0.13.5-alpha"
    assert altr_stream.__version__ == "0.13.5-alpha"
...
async def test_api_version_endpoints():
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        ...
        assert root_data["version"] == "0.13.5-alpha"

        # Health endpoint
        health_res = await client.get("/api/v1/health")
        assert health_res.status_code == 200
        health_data = health_res.json()
        assert health_data["version"] == "0.13.5-alpha"
...
def test_settings_version_override_explicit_and_env(monkeypatch):
    ...
    assert s_empty.app_version == "0.13.5-alpha"
    ...
    assert s_env_empty.app_version == "0.13.5-alpha"
