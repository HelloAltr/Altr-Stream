"""MySQL physical query lowerer for AltrQL.

Translates schema-bound BoundAltrQueryIR into deterministic, fully parameterized
MySQL SELECT, INSERT, UPDATE, and DELETE statements with '%s' parameter markers.
"""
...
def _convert_literal_to_parameter(lit: LiteralValue) -> Any:
    """Convert an AltrQL literal value into a Python native parameter for MySQL drivers."""
...
class MySQLLowerer(QueryLowerer):
    """Deterministic physical query compiler for MySQL dialect."""

    def lower(self, query: BoundAltrQueryIR) -> PhysicalQueryResult:
        """Translate a schema-bound AltrQL query into a parameterized MySQL PhysicalQuery or PhysicalQueryBatch."""
        def quote_ident(name: str) -> str:
            escaped = name.replace("`", "``")
            return f"`{escaped}`"

        # Target table identifier (with namespace qualification if present and not default)
        if query.entity.namespace and query.entity.namespace.lower() != "default":
        ...
        if query.operation == QueryOperation.CREATE:
            ...
            for group in groups:
                ...
                group_sql = f"INSERT INTO {target_table} ({', '.join(cols)}) VALUES {', '.join(row_value_tuples)};"
        ...
        def lower_field_path(field: BoundFieldPath) -> str:
            ...
            return f"JSON_UNQUOTE(JSON_EXTRACT({root}, '{json_path}'))"
        ...
        def lower_field_expression(expr: BoundFieldExpression) -> str:
            ...
            if isinstance(op, ComparisonOperator):
                ...
                # CompoundAndConstraint operand: (col >= %s AND col <= %s)
                ...
                if isinstance(operand, ValueSet):
                    ...
                    if op == ComparisonOperator.EQ:
                        ...
                        # Group simple scalar literals into IN (%s, %s, ...)
                        simple_scalars = [
            ...
            raise NotImplementedError(f"Unsupported operator '{op}' in MySQLLowerer")
        ...
        if query.operation == QueryOperation.UPDATE:
            ...
            final_sql = f"UPDATE {target_table} SET {', '.join(set_items)}{where_sql};"

        elif query.operation == QueryOperation.DELETE:
            where_sql = f" WHERE {lower_expression(query.where)}" if query.where is not None else ""
            final_sql = f"DELETE FROM {target_table}{where_sql};"
